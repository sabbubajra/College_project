...
df= pd.read_csv('C:/Users/DREAM/Desktop/project/churn/dataset.csv')
#print(df)
columns=['tenure','MonthlyCharges','TotalCharges','gender','SeniorCitizen','Partner','Dependents','PhoneService','MultipleLines','OnlineSecurity','OnlineBackup','DeviceProtection','TechSupport','StreamingTV','StreamingMovies','PaperlessBilling']
df2=df.drop(columns,axis=1,inplace=True)
#print(df)
#df=df.sample(n=10)
#df=df1.sample(n=7)
df=df.head()
...
def find_entropy_attribute(df,attribute):
     ...
     return abs(entropy2)
#print(find_entropy_attribute(df,'InternetService'))
...
data={"InternetService":"Fiber optic","Contract":"One year","PaymentMethod":"Electronic check"}
inst=pd.Series(data)
#int=for i in data
#inst=data.values()
print(data)
...
dataset
columns=['MonthlyCharges','TotalCharges','gender','SeniorCitizen','Partner','Dependents','DeviceProtection','PaperlessBilling']
...
def train_test_split(dataset):
    training_data = dataset.iloc[:6000].reset_index(drop=True)  # We drop the index respectively relabel the index
...
data={"tenure":25,"InternetService":"DSL","MultipleLines":"Yes","Contract":"Month-to-month","PaymentMethod":"Electronic check",
